...
export default class ThumbRaiser {
    constructor(generalParameters, audioParameters, cubeTexturesParameters, mazeParameters, playerParameters, ambientLightParameters, directionalLightParameters, spotLightParameters, flashLightParameters, shadowsParameters, fogParameters, collisionDetectionParameters, fixedViewCameraParameters, firstPersonViewCameraParameters, thirdPersonViewCameraParameters, topViewCameraParameters, miniMapCameraParameters) {
        ...
        this.loadinFloor = false;
        this.chosingFloor = false;
        this.goingToAnotherPiso = "";
        ...
        this.renderer = new THREE.WebGLRenderer({ antialias: true, canvas: this.generalParameters.canvas });
    ...
    switchToAnotherBuildingFloor(nextPiso) {
        this.scene.remove(this.maze);
        
        let newMaze = {
            url: "./mazes/Edificio_" + nextPiso.edificio + "_Piso_" + nextPiso.piso + ".json",
            ...
            startLocation: nextPiso.startLocation,
        }
        this.mazeParameters = newMaze ;
        this.maze = null;
        this.maze = new Maze(this.mazeParameters);
        
        //while(!this.maze.loaded) {console.log("Loading maze...")};
        
        
        document.getElementById("floor-title").textContent = "Edificio " + nextPiso.edificio + " - Piso " + nextPiso.piso;
        document.getElementById("floor-title").style.display = "flex";

        setTimeout(() => {
            document.getElementById("floor-title").style.display = "none";
        }, 3000);
        
        
        this.scene.add(this.maze);
        this.scene.remove(this.player);
        this.player.direction = nextPiso.initialDirection;
        this.scene.add(this.player);
        this.renderer.render(this.scene, this.activeViewCamera.activeProjection);
    }

    elevatorClick(floorSelection) {
        this.goingToAnotherPiso = {"found": true, "edificio": this.maze.elevador.edificio, "piso": floorSelection, "startLocation": this.maze.elevador.startLocation, "initialDirection": this.maze.elevador.initialDirection};
       
        this.floorChoosed = true;
    }

    viewBuildingFloor(buildingFloor){
        this.scene.remove(this.maze);
        
        let newMaze = {
            url: "./mazes/" +buildingFloor+".json",
            designCredits: "Maze designed by Cecília Fernandes and Nikita.",
            texturesCredits: "Maze textures downloaded from <a href='https://www.texturecan.com/' target='_blank' rel='noopener'>TextureCan</a>.",
            scale: new THREE.Vector3(1.0, 1.0, 1.0),
            helpersColor: new THREE.Color(0xffffff),
        ...
        document.getElementById("floor-title").textContent = buildingFloor;
        document.getElementById("floor-title").style.display = "flex";
    ...
    update() {
        if (!this.gameRunning) {
            ...
            if (!this.animations.actionInProgress) {
                ...
                if (this.maze.foundExit(this.player.position).found) {
                    this.switchToAnotherBuildingFloor(this.maze.foundExit(this.player.position));
                    this.loadinFloor = true;
                }
                else if (!this.chosingFloor && this.maze.foundElevator(this.player.position).found) {
                    
                    let floorSelection = document.getElementById('floor-buttons');
                    floorSelection.innerHTML = ''; // Clear the current options
                    this.maze.elevador.pisos.forEach(piso => {
                        ...
                        option.style.marginRight = "6px";
                        option.style.backgroundColor = "white";
                        option.style.color = "#FE904E";
                        option.style.border = "none";
                        option.style.borderRadius = "10px";
                        option.style.boxShadow = "0px 0px 2px 2px rgb(0,0,0)";

                        floorSelection.appendChild(option);
                        option.onclick= () => this.elevatorClick(piso);
                    });

                    // Show the elevator panel
                    document.getElementById('elevator-panel').style.display = 'flex';
                    // Add an event listener to the "Go" button
                    this.chosingFloor=true;
                    this.floorChoosed=false;
                }
                else if(this.chosingFloor && this.floorChoosed){
                    this.switchToAnotherBuildingFloor(this.goingToAnotherPiso);
                    this.loadinFloor = true;
                    this.chosingFloor = false;
                    document.getElementById('elevator-panel').style.display = 'none';
                }
                else {
